...
class IconBrowser(UserControl):
    ...
    def build(self):
        ...
        def copy_to_clipboard(e):
            icon_name = e.control.data
            print("Copy to clipboard:", icon_name)
            self.page.set_clipboard(e.control.data)
            self.page.show_snack_bar(
                SnackBar(Text(f"Copied {icon_name}"), open=True))
        ...
        def display_icons(search_term: str):
            ...
            if len(search_results.controls) == 0:
                self.page.show_snack_bar(
                    SnackBar(Text("No icons found"), open=True))
